...
# Author mix: Zahid Islam (udp-zivpn) + tweaks + M-69P UI polish
# Features updated: Automatic account deletion on expiration, Notification card with user details for 20 seconds, and Auto-detected Server IP in Web UI.
# Myanmar fonts fixed (web.py)
#ဖန်တီးသူ TINT AUNG KHAING
set -euo pipefail
...
say "${G}🔏 VPN Password List (ကော်မာဖြင့်ခွဲ) eg: M-69P,tak,dtac69${Z}"
...
say "${Y}🖥️ Web Panel (Flask) ကို ထည့်နေပါတယ် (Auto-Delete, Card Notif & Auto-IP ထည့်သွင်းထားသည်)${Z}"
...
# 🛑 အသစ်ထည့်သွင်းခြင်း: IP Address ကို ရယူရန် function
def get_server_ip():
    try:
        # 'ip route get 1' ကိုသုံးပြီး Default Interface ရဲ့ IP ကို ယူခြင်း
        # သို့သော်၊ ဤနည်းလမ်းသည် တစ်ခါတစ်ရံ VPN tunnel ကြောင့် မှားနိုင်သဖြင့်
        # hostname -I ကို သုံးခြင်းသည် အလွယ်ဆုံးနှင့် အများအားဖြင့် ပိုမှန်သည်
        result = subprocess.run(['hostname', '-I'], capture_output=True, text=True, check=True)
        ip = result.stdout.strip().split()[0] # ပထမဆုံး IP ကိုသာယူ
...
<body>
    <div class="login-container">
        <div class="profile-image-container">
            <img src="{{logo}}" alt="M-69P Profile" class="profile-image">
        </div>
        <h1>M-69P</h1>
...
<form action="/login" method="POST" class="login-form">
        ...
        {% if IP %}
        <p class="panel-title" style="margin-top:15px;font-size:0.85em;color:#007bff;">Server IP: {{ IP }}</p>
        {% endif %}
    </div>
</body>
{% else %}
...
<form method="post" action="/add" class="boxa1">
  ...
  <div class="row">
    ...
    <div class="input-group">
     <div><label><span class="icon">🔥</span>Server IP</label><input name="ip" placeholder="ip" value="{{ IP }}" readonly></div></div>
...
def build_view(msg="", err=""):
  # ✅ ပြင်ဆင်ချက်: IP variable ကို အသုံးပြုရန်
  server_ip = SERVER_IP_FALLBACK 
  
  if not require_login():
    # Login Guard logic ကို ဖြတ်သန်းခြင်း မရှိသေးပါက
    return render_template_string(HTML, 
                                  authed=False, 
                                  logo=LOGO_URL, 
                                  err=session.pop("login_err", None),
                                  # ✅ ပြင်ဆင်ချက်: Login Page သို့ IP ကို ပေးပို့ခြင်း
                                  IP=server_ip) 
  ...
  # Login ဝင်ပြီးနောက် HTML template သို့ ပေးပို့ခြင်း
  return render_template_string(HTML, 
                                authed=True, 
                                logo=LOGO_URL, 
                                users=view, 
                                ...
                                err=err, 
                                today=today,
                                # ✅ ပြင်ဆင်ချက်: Authenticated View သို့ IP ကို ပေးပို့ခြင်း
                                IP=server_ip)
...
def login():
  ...
  # GET
  # ✅ ပြင်ဆင်ချက်: Login GET request မှာလည်း IP ကို ပေးပို့ခြင်း
...
def add_user():
  ...
  # IP ကို form ကနေ ယူတယ်၊ မပါရင် server IP ကို ယူတယ်
  ...
  msg_dict = {
      ...
      "password": password,
      "expires": expires,
      # ✅ ပြင်ဆင်ချက်: User Info Card အတွက် IP ကို ထည့်သွင်းခြင်း
      "ip": ip 
  }
